...
def grid_selectors(dim: list[int, int], zoom = 1, deplacement_map = (0,0)) -> None:
    ...
    for i in range(-delta_dim_y, dim[1] + delta_dim_y):
        for j in range(-delta_dim_x, dim[0] + delta_dim_x):
                i_, j_ = i, j
...
def grid(dim: list[int, int], zoom:float = 1, color: str = 'blue') -> None:
    ...
    # now use fltk.ligne
    for i in range((h - grid_height)//(-unit*2), dim[1] + 1 + (h - grid_height)//(unit*2)):
        idx = i
